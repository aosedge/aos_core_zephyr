...
Error JSONProvider::ParseNodeConfig(const String& json, sm::resourcemanager::NodeConfig& config) const
{
    ...
    memset(parsedNodeConfig.Get(), 0, sizeof(NodeConfig));

    parsedNodeConfig->version  = "";
    parsedNodeConfig->nodeType = "";
    parsedNodeConfig->priority = 0;

    parsedNodeConfig->devicesLen   = 0;
    parsedNodeConfig->resourcesLen = 0;
    parsedNodeConfig->labelsLen    = 0;
