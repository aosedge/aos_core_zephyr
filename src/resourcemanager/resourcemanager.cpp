...
static Error FillAosStruct(const NodeConfig& in, Array<ResourceInfo>& outResources)
{
    ...
    for (size_t i = 0; i < in.resourcesLen; ++i) {
        ...
        for (size_t i = 0; i < inResourceInfo.mountsLen; ++i) {
            const auto& parsedMount = inResourceInfo.mounts[i];

            aos::FileSystemMount& mount = outResourceInfo.mMounts[i];

            mount.mDestination = parsedMount.destination;
            mount.mType        = parsedMount.type;
            mount.mSource      = parsedMount.source;

            for (size_t j = 0; j < parsedMount.optionsLen; ++j) {
                if (auto err = mount.mOptions.PushBack(parsedMount.options[j]); !err.IsNone()) {
                    return AOS_ERROR_WRAP(err);
                }
            }
        }
        ...
        for (size_t i = 0; i < inResourceInfo.hostsLen; ++i) {
            outResourceInfo.mHosts[i].mIP       = inResourceInfo.hosts[i].ip;
            outResourceInfo.mHosts[i].mHostname = inResourceInfo.hosts[i].hostName;
        }
    }
