...
aos::Error OCISpec::LoadImageSpec(const aos::String& path, aos::oci::ImageSpec& imageSpec)
{
    ...
    auto redRet = ReadFileContentToBuffer(path, cJsonMaxContentSize);
    if (!redRet.mError.IsNone()) {
        return redRet.mError;
    }
...
aos::RetWithError<int> OCISpec::ReadFileContentToBuffer(const aos::String& path, size_t maxContentSize)
{
    aos::RetWithError<int> err(0, aos::ErrorEnum::eNone);

    auto file = open(path.CStr(), O_RDONLY);
    if (file < 0) {
        err.mError = AOS_ERROR_WRAP(file);
        return err;
    }

    err.mValue = read(file, mJsonFileBuffer.Get(), maxContentSize);
    if (err.mValue < 0) {
        err.mError = AOS_ERROR_WRAP(err.mValue);
    }

    auto ret = close(file);
    if (err.mError.IsNone() && (ret < 0)) {
        err.mError = AOS_ERROR_WRAP(ret);
        err.mValue = ret;
    }
