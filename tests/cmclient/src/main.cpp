...
void testImageDownloadInstances()
{
    ...
    zassert_true(strcmp(gReceivedMessage.SMOutgoingMessage.image_content_request.content_type, "service"),
        "Incorrect request content type");
    zassert_true(
        strcmp(gReceivedMessage.SMOutgoingMessage.image_content_request.url, request.url.CStr()), "Incorrect url");
